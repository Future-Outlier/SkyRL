...
async def test_request_identity_preserves_native_generate_route(isoexec):
    ...
    client._post = AsyncMock(return_value={"choices": [{"token_ids": [5], "finish_reason": "length"}]})
    ...
    if isoexec:
        ...
        assert call.kwargs["headers"]["X-Request-Id"] == call.kwargs["json"]["request_id"]
    else:
...
async def test_step_capture_collects_every_engine_rank(tmp_path, monkeypatch, verdict):
    ...
    assert all(call.kwargs["json"] == {"method": "isoexec_refusal_end_step"} for call in session.request.call_args_list)


def test_trainer_gate_retains_batch_alignment_and_exports_before_refusal(tmp_path, monkeypatch):
    import json
    ...
    trainer._execute_forward_pass = Mock(return_value=torch.tensor([[0.0, -1.0, -2.0], [-1.0, -2.0, -3.0]]))
    exports = []
...
async def test_batched_generator_carries_unique_sessions_only_during_capture(monkeypatch, active_capture):
    from types import SimpleNamespace
    ...
    tids = [SimpleNamespace(instance_id="same-prompt", repetition_id=i) for i in range(2)]
    ...
    assert engine_input["session_ids"] == (["same-prompt_0", "same-prompt_1"] if active_capture else None)
