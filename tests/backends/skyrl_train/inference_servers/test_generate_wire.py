...
def test_full_logprob_codec_delegates_to_the_installed_package(monkeypatch):
    import sys
    from types import SimpleNamespace
    from unittest.mock import Mock

    payload = {"data": "", "shape": [0, 3], "dtype": "float32"}
    rows = np.empty((0, 3), dtype=np.float32)
    pack = Mock(return_value=payload)
    decode = Mock(return_value=rows)
    monkeypatch.setitem(
        sys.modules,
        "isoexec.integrations.full_distribution",
        SimpleNamespace(pack_full_logprobs=pack, decode_packed_full_logprobs=decode),
    )
    assert pack_full_logprobs([], [], vocab_size=3) is payload
    assert decode_packed_full_logprobs(payload) is rows
    pack.assert_called_once_with([], [], 3)
    decode.assert_called_once_with(payload)
